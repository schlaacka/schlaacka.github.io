window.loadLeverJobs = function (options) {
  ...
  function createJobs(responseData) {
    ...
    for(var i = 0; i < groupedPostings.length; i++) {
      ...
      if (haveDepartments) {
        content += '<br><br> <section class="lever-department" data-department="'+ groupedPostings[i].departmentTitle + '"><p class="lever-department-title">' + sanitizeForHTML(groupedPostings[i].departmentTitle) + '<br><br>';
      };
...
for (var k = 0; k < groupedPostings[i].teams[j].postings.length; k++) {
  ...
  const fixedJobTitle = sanitizeForHTML(groupedPostings[i].teams[j].postings[k].text).replace("&amp;", "&");
